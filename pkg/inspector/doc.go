...
// [logstore.RequestEvent]: id, optional source ("ingest" or "replay"), request, response, durationMs. Nested [logstore.Request] and
...
//
//	{"id":"…","source":"ingest","request":{"method":"GET","path":"/","body":null,"headers":{…}},"response":{"durationMs":0,"statusCode":200,"headers":{…},"body":…},"durationMs":12}
//
...
//
// GET /logs — JSON array of stored events. GET /log?id=… — single event. POST /replay — proxies
// to localhost (SSRF-safe). Optional request header X-Inspector-Log-Replay: 1 (or true) stores the
